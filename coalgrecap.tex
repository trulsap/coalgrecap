...
\textbf{Covariant powerset functor (aka \emph{wtf?})} is a~functor $P: \cat{Set} \to \cat{Set}$ which takes a~map $f: X \to Y$ to a~map $P(f): P(X) \to P(Y)$ with $(Pf)(U) = f[U]$.\footnote{\piotr{Found this in some crazy paper, and I've no idea what it means. My own notes make no sense, so no help there. }}
