...
\begin{proposition} About $Equ(A)$
  ...
  \item \textbf{join:} the join of $\equiv_1$ and $\equiv_2$ is the equivalence
    class $\equiv$ generated by $\equiv_1 \cup \equiv_2$.
  \end{itemize}
\end{proposition}
...
\begin{definition}[Equalizer]
  \label{def:equalizer}
  An equalizer of two parallel morphisms $f, g : A \rightarrow B$ in a category C consists
  of an object E and a morphism $m : E \rightarrow A$ with $m; f = m; g$.
  Moreover, the pair $(E, m)$ satisfies the following universal
  property\footnote{This universal property essentially says that E must be as
    big as possible.}:
  \begin{itemize}
  \item For any other object X and morphism $f_x : X \rightarrow A$ with $f_{x} ; f = fx ; g$
    there exists a unique morphism $k : X \rightarrow E$ such that $k; m = f_x$.
  \end{itemize}
...
\begin{definition}[CoEqualizer]
  \label{def:equalizer}
  An equalizer of two parallel morphisms $f, g : A \rightarrow B$ in a category C consists
  of an object C and a morphism $c : B \rightarrow C$ with $ f;m = g;m$.
  Moreover, the pair $(C, c)$ satisfies the following universal property:
  \begin{itemize}
  \item For any other object D and morphism $h : B \rightarrow D$ with $f; h = g ; h$
    there exists a unique morphism $k : C \rightarrow D$ such that $c; k = h$.
  \end{itemize}
  Picture: \xymatrix{
    A \ar@/^/[r]^{f} \ar@/_/[r]_{g} & B\ar[r]^{c}\ar[dr]^{h}  & C\ar@{.>}[d]^{\exists !k}\\
    & & D }
\end{definition}

I stole the following from the internet:

Suppose B is a set and $R ⊆ B \times B$ is an equivalence relation. We denote the
equivalence class of $b \in B$ wrt. R by [b]; $c : B \to B/R$ is the canonical
projection. Then (B/R, c) is a coequaliser diagram over the projections $\pi_1$
and $\pi_2$, both mapping $R \to B$.

In general however, the arrows of which we construct coequalisers are not the
projections of an equivalence relation. The construction of coequalisers in the
general case takes the quotient by the generated equivalence relation. This
works as follows:
...
The point of this execise is that the existence of such a homomorphism shows that the function \textit{can be defined by recursion} and the \textit{uniqueness yields a proof by induction.} [Larry]

\begin{example}Let's define the function $2 * \_ : \nn \to \nn$! 
\begin{description}
\item[Old school] Base case: $2 * 0 := 0$; induction step: $2 * s(n) := (2 * n) + 2$.
\item[21st century] ``Make it reign!'' (Paraphrasing Tom Waits)
\begin{figure}[ht]
\begin{minipage}[b]{0.4\linewidth}
\begin{center}
\begin{tikzpicture}
